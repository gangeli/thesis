...
\longcaption{A proof in natural logic for \textit{all tabby cats have tails} from
    the premise \textit{all cats have tails}.}{\label{fig:catsproof}Note that 
  $\le_t$ is equivalent to entailment ($\Rightarrow$).
...
\begin{table}
	...
	\end{tabular}
	%(caption)
	\longcaption{The natural logic projection table.}{The projection table is between a phrase $e_1$ and
...
\begin{table}
	...
	\end{tabular}
	%(caption)
	\longcaption{The natural logic join table.}{Note that the $\independent$ always joins to yield $\independent$,
...
\begin{center}
  ...
  \end{tabular}
\end{center}
\longcaption{The natural logic join table expressed as a finite state automata.}{\label{fig:fsa}(a) The join table in \reftab{join} expressed as a finite state automata.
