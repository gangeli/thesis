...
\begin{table}[t]
	...
	\end{tabular}
	%(caption)
	\longcaption{The join table as taken from \newcite{key:2012icard-natlog}.}
  {
...
\begin{center}
  ...
  \end{tabular}
\end{center}
\longcaption{Natural logic inference expressed as a (possibly collapsed) finite state automaton.}
{
...
\bottomrule
\end{tabular}
\longcaption{A tabular natural logic proof negating \textit{no carnivores eat animals} from \textit{the cat ate a mouse}.}
...
\bottomrule
\end{tabular}
\longcaption{Monotonicity for quantifiers marked with additivity / multiplicativity information}
...
If we return to our naive inference from the beginning of the section 
  which incorrectly inferred that  \ww{every cat has a tail} negates \ww{every dog has a tail},
