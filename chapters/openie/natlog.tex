...
Analogously, \ww{Alice played baseball on Sunday} 
  entails that \ww{Alice played
  on Sunday}; but, \ww{Obama signed the bill on Sunday} should not entail the
...
\end{tabular}
\end{center}
\longcaption{Representative examples from the six dependency patterns used to segment an atomic sentence into an 
  open IE triple.}
{\label{tab:patterns}
...
\end{tabular}
\end{center}
\longcaption{Representative examples from the eight patterns used to segment a noun phrase into an open IE
  triple.}
{\label{tab:nominal}
...
% Simplification important in its own right (DanJ)
Although the focus of this chapter is on open IE and generating relation triples,
...
Later work began to also focus more on sentence simplification for
  \textit{summarization}
For example, \newcite{key:2000knight-simplification} approached the task with
  a noisy-channel model; \newcite{key:2008clarke-simplification} introduced an
...
Work in this area in the mid 2010's, in turn, has tended to frame the task as
  ...
  neural networks (e.g., \newcite{key:2015rush-simplification}).
